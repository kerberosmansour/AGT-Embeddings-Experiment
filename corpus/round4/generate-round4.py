#!/usr/bin/env python3
"""Round-4 held-out corpus generator for embeddings/security classification.

This generator is intentionally separate from corpus/generate.py. The files here
must not be merged into corpus/contrib because Round-4 is measuring overfitting,
false positives, and base-rate precision; contaminating the exemplar bank would
defeat the point of the experiment.
...
def multilingual_family_id(attack_class: str, benign_subclass: str, text: str) -> str:
    ...
    if attack_class == "benign":
        return f"r4:benign:{benign_subclass}:multilingual:{normalized_hash}"
    return f"r4:{attack_class}:multilingual:{normalized_hash}"
...
def render_attack(attack_class: str, template: str, idx: int) -> str:
    ...
    canary = f"SYS-CANARY-R4-{idx:04d}"
    ...
    return f"{rendered} Scenario R4-A-{idx:05d}. Trace {digest}."
...
def render_compact_benign(idx: int) -> tuple[str, int]:
    ...
    return f"{COMPACT_BENIGN_BASES[template_idx]} Reference R4-C-{idx:05d}. Trace {digest}.", template_idx
...
def render_benign(subclass: str, template: str, idx: int) -> str:
    ...
    return f"{rendered} Reference R4-B-{idx:05d}. Trace {digest}."
...
def build_rows(profile: str) -> list[dict]:
    ...
    for attack_class, templates in ATTACK_TEMPLATES.items():
        ...
        while emitted < limit:
            ...
            family_id = f"r4:{attack_class}:template:{template_idx:05d}"
    ...
    while emitted < compact_limit:
        ...
        family_id = f"r4:{attack_class}:compact_obfuscation:{template_idx:05d}"
    ...
    for subclass, templates in BENIGN_TEMPLATES.items():
        ...
        while emitted < limit:
            ...
            family_id = f"r4:benign:{subclass}:template:{template_idx:05d}"
    ...
    while emitted < limit:
        ...
        base = f"{BENIGN_OBFUSCATION_BASES[template_idx]} Reference R4-O-{idx:05d}. Trace {digest}."
        family_id = f"r4:benign:{subclass}:template:{template_idx:05d}"
...
def write_manifest(
    ...
    manifest = {
        ...
        "gates": {
            "placement": "corpus/round4 only; never corpus/contrib",
...
def main() -> int:
    ...
    corpus_path = Path(args.out) if args.out else HERE / f"injection-round4-{args.profile}.jsonl"
    manifest_path = Path(args.manifest) if args.manifest else HERE / f"manifest-{args.profile}.json"
